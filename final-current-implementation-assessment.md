...
하지만 “프로덕션 공개 가능”이라고 말하기에는 아직 이르다. 기능은 꽤 갖춰졌지만, 운영 자동화, 실제 OCR 품질 검증, 전체 의약품/DUR 데이터 커버리지, 실제 FCM 단말 검증, 의료/법무 검토가 부족하다.
...
- `sync-dur-interactions`는 최신 DUR 명세 기준 endpoint로 수정됐고 `version 4`까지 배포됐다.
- `gemini-chat`은 상호작용 질문에서 근거 부족 시 Gemini 추론을 막는 안전 가드가 들어갔다.
...
- 아직 전체 451개 약품에 대해 DUR batch를 모두 돌린 것은 아니다.
- 정기 스케줄러가 실제 등록된 상태는 아니다.
- 공공 API quota, timeout, 장애 시 재시도 전략이 부족하다.
- 어떤 offset까지 처리했는지 추적하는 별도 job state 테이블이 없다.
...
지금은 함수가 있지만 scheduled job이 실제 운영에 등록된 상태가 아니다.

필요한 것:

- `sync-drug-master` 정기 실행
- `sync-dur-interactions syncKnownMedications=true` 정기 실행
- `send-medication-reminders` 정기 실행
- `redact-expired-sensitive-data` 정기 실행
- 실패 알림
...
| DUR 상호작용 | 보통 이상 | 최신 endpoint 수정 및 batch 적재 가능, 전체 운영은 미완 |
| 챗봇 안전성 | 좋음 | 근거 부족 시 차단 가드 존재 |
| 복약 일정/로그 | 좋음 | 기본 플로우 원격 통과 |
| FCM | 보통 이하 | dry-run 수준, 실제 단말 검증 필요 |
| 보호자 | 보통 | 최소 기능만 있음 |
| 운영 자동화 | 부족 | scheduled job 미등록 |
